...
function MachineLearningBody(x, plt, ax, temperature, path)
    ...
    while temperature <= FinalTemp
        ...
        for iCount = 1:MonteCarloSweeps
            ...
            ax.set_xlabel("Temperature = " * string(temperature))
            ik = ax.text(0.05, 0.85, "Temp="*string(temperature),  transform=ax.transAxes, fontsize=7 ,bbox=props)
            im = plt.imshow(x)
            Directory = path * "pic" *string(temperature)*"_"*string(iCount)*".png"
