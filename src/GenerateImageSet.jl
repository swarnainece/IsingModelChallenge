...
function GenerateImageSet(x, plt, temperature, path)

    iCount = 1
    arrayToStore_x = []

    while temperature >= FinalTemp   # CHange the relational operator according to temp
        ...
        #push Images & Save Images
        #println(x)
        #push!(imageArrayMatrixOriginal, x)      #Push updated x into imageArrayMatrixOriginal in the form of matrix

        plt.imshow(x)
        plt.xlabel("Temperature = " * string(temperature))
        Directory = path * "pic" * string(iCount) * ".png"
        #Directory = Directory * "pic" * string(iCount) * ".png"
        #println(Directory)
        plt.savefig(Directory)

        #Test Check for the magnetization value
    ...
    #println(arrayToStore_x)
    #return arrayToStore_x
end
